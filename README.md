...
默认使用网站用户名和密码登录。第一次点击“新建用户”时，需要先用一个在允许名单中的网易云账号完成验证，然后设置网站账号；网站账号与该网易云 `userId` 一对一绑定，暂不支持换绑。网易云验证优先使用当前网页扫码登录接口，也保留短信验证码作为备用方式。

项目使用 `instance/allowlist.json` 保存允许使用的网易云用户 ID 和权限等级。名单为空时，第一个成功注册的网站账号对应的网易云账号会自动成为管理员；之后只有名单内账号可以创建网站用户并使用功能。管理员可在右上角账号窗口中搜索、添加和删除普通用户，普通用户不能访问名单管理。

网站账号保存在 `instance/accounts.json`，网易云绑定 Cookie 保存在 `instance/netease_bindings.json`。这些文件都属于登录凭据，不要分享或提交到公开仓库。

网站账号登录后，即使网易云绑定 Cookie 暂时失效，仍可访问服务器已有资源，并可尝试搜索和下载免费歌曲。下载需要账号权益的歌曲时，页面会提示重新验证原来绑定的网易云账号。
