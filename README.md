...
- 第一个成功注册的网易云账号会成为管理员。完成初始化后，只把可信任的网易云账号加入允许名单，不要把管理入口和 Cookie 分享给他人；当前允许名单成员共享 `outputs/` 中的素材、生成视频和全局视频队列，不提供按用户隔离；
- 应用本身不是完整的公网 SaaS 防护层，登录、验证码、扫码和视频接口应在反向代理或防火墙增加访问控制、日志、限流和必要的请求大小/超时限制；
- Cookie 导入会把浏览器导出的网易云 Cookie 发送到你部署的服务器，并写入 `docker-data/instance`。请只在你控制的 HTTPS 端点使用；
- 登录二维码图片目前通过 `api.qrserver.com` 生成，扫码设备指纹脚本从 `st.music.163.com` 加载。这样会向第三方发送二维码登录 URL、浏览器请求和部分设备信息；
- `docker-data/outputs` 可能包含音频、歌词、封面和视频。发布源码压缩包、备份或镜像构建上下文前，确认没有把 `instance/`、`outputs/` 或 `docker-data/` 一并带出。
